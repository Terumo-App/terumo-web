...
import { FiBookmark, FiMoreVertical, FiTrash, FiTrash2 } from 'react-icons/fi';
  ...
  useEffect(() => {
    fetch(fakeDataUrl)
      ...
      .then((res) => {
        ...
        setData(
          aux.sort((a, b) => Number(a.score) - Number(b.score))
        );
        setList(
          aux.sort((a, b) => Number(a.score) - Number(b.score))
        );
  ...
  const loadMore =
    !initLoading && !loading ? (
      <div
        ...
        <ButtonPrimary onClick={onLoadMore}>
          Loading More
        </ButtonPrimary>
      </div>
    ) : null;
            ...
            cover={
              <>
                <div
                  ...
                  <FiBookmark style={{ width: 25, height: 25 }}/>
                  <FiTrash2 style={{ width: 25, height: 25 }} />
                  <Dropdown
                    menu={{
                      items,
                    }}>
